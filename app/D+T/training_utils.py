...
class CustomDataGen():    
    ...
    def data_generator(self):
        for patient in self.patients:
            image = load_nii(f'{self.data_path}/images/{patient}.nii.gz') # H,W,D,T
            mask = load_nii(f'{self.data_path}/masks/{patient}.nii.gz').astype('uint8') # H,W,D,T
            H, W, D, T = image.shape

            image = image.reshape(H, W, D * T)  # shape: (H, W, D*T)
            mask = mask.reshape(H, W, D * T)    # shape: (H, W, D*T)
...
def evaluate(model_name, all_patients, run, data_path, output_signature):
    ...
    for cohort in cohorts:
        ...
        for patient in all_patients[cohort]:
            ...
            pred_masks = model.predict(test_ds)[-1][0]
            pred_masks = get_one_hot(np.argmax(pred_masks,axis = -1), 3)

            images, true_masks = next(iter(test_ds))
            images = np.array(images[0])
            true_masks = np.array(true_masks[0])

            X = images.shape[2]
            Y = X//32
            images = images.reshape(128,128,Y,32,1)
            true_masks = true_masks.reshape(128,128,Y,32,3)
            pred_masks = pred_masks.reshape(128,128,Y,32,3)


            for frame in [0,16]:
                image = images[:,:,:,frame,:]
                true_mask = true_masks[:,:,:,frame,:]
